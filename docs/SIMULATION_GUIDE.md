...
- **动力学双轮自行车模型（v0.1.1+）**：自车具备完整的轮胎侧偏动力学（侧偏角→侧向力→
  横摆角速度→航向角），带松弛长度（τ=0.3/vx）和摩擦圆约束（μ=0.9）。低速自动退化到
  运动学模型。EPS 转向执行器含一阶低通滤波（τ≈0.15s）模拟真车转向机惯性。
  通过 `heading` / `steer` 控制，可演示真实带 sideslip 的**变道**和**弯道保持**。
