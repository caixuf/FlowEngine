...
> 基于 Phase 4 端到端验证中暴露的问题和用户反馈，优先推进以下工作。

| 优先级 | 任务 | 文件 | 说明 |
|--------|------|------|------|
| 🔴 P0 | **场景格式统一迁移** | `scenarios/*.json`, `flowsim_node.cpp` | 14 个旧场景从 `x/y` 迁移到 `s/l/segment_id`；删除 flowsim 中旧格式兼容路径（`else` 分支），统一为 Frenet→World 单一路径 |
| 🔴 P0 | **perception↔sensor 数据流对接** | `perception_node.cpp`, `pipeline.json` | perception 从 `vehicle/state`（真值作弊）改为订阅 `sensor/lidar`，打通传感器噪声/FOV/遮挡链路 |
| 🟡 P1 | **planning 超车能力** | `planning_node.cpp` | ego 被同车道慢车（vx=3.0）阻挡时无法变道超车；需在 `lane_change` route step 基础上支持基于障碍物的主动变道决策 |
| 🟡 P1 | **分叉道路 3D 渲染** | `tools/flowboard/js/scene3d.js` | `road_network` 含 junctions 时，前端仅渲染顺序 edges，不显示分叉/汇入路网拓扑 |
| 🟡 P1 | **规划轨迹 3D 渲染** | `scene3d.js` | 前端不显示 planning 下发的参考轨迹线 |
| 🔵 P2 | **esmini 加载失败兜底** | `flowsim_node.cpp` | `g.roads_loaded=false` 时所有新格式 NPC 落入旧路径（x/y=0→全部堆在原点）。应改为至少用线性累积 x=s 放置 |
| 🔵 P2 | **逆向车 heading 冲突** | `flowsim_node.cpp` | `e.heading=M_PI` 无条件覆盖 esmini heading；若逆向车在弯道段可能朝向错误。当前仅交叉口直道使用，低风险 |
