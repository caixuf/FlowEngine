...
> **状态：收尾完成（2026-07-18）。** Phase 1-6 全部完成（13/13）。
> city_to_highway_full 为默认旗舰场景，24 NPC + 分叉/汇入 + 超车全链路可运行。
> 仅剩：3D LiDAR 真实点云接入。
>
...
| 3D 车道宽度不准确 | per-road lane_width 查询 | Phase 6 |
| 逆向车 heading 覆盖 | fmod(heading+π, 2π) 叠加 | Phase 6 |
...
### Phase 6 — 质量收敛与能力补全 ✅ 全部完成 (2026-07-18)

> **13/13 项已完成。** Phase 1-6 主体交付完毕。
...
| 🟡 P1 | **3D 车道渲染修正** | `scene3d.js`, `scene_pub.cpp` | 道路渲染与车辆位置视觉上不匹配 | ✅ scene_pub 改为 `cfg.roads->lane_width(road_id, -1, 0.0)` per-road 查询实际宽度，失败退回 cfg.lane_width |
| 🟡 P1 | **planning 超车能力** | `control_node.cpp` | ego 被同车道慢车阻挡时无法变道超车 | ✅ control 已有被动超车状态机（lc_state 0→1→2→3），Phase 6 补全感知后自动生效 |
| 🟡 P1 | **分叉道路 3D 渲染** | `scene3d.js` | `road_network` 含 junctions 时，前端仅渲染顺序 edges | ✅ 前端端点聚类检测 junction，fork 标橙色环、merge 标绿色环 |
| 🟡 P1 | **规划轨迹 3D 渲染** | `scene3d.js`, `monitor_node.c` | 前端不显示 planning 下发的参考轨迹线 | ✅ monitor 透传 trajectory path，scene3d 画蓝色轨迹线 |
| 🔵 P2 | **demo.sh topology 文件保留** | `scripts/demo.sh` | cleanup 中 rm -f 导致事后评估无数据 | ✅ 改为保留+时间戳副本 |
| 🔵 P2 | **esmini 加载失败兜底** | `flowsim_node.cpp` | `g.roads_loaded=false` 时 NPC 全堆原点 | ✅ segment_id≥0 线性放置 (x=s, y=l+road_center_y) |
| 🔵 P2 | **逆向车 heading 冲突** | `flowsim_node.cpp` | `e.heading=M_PI` 覆盖 esmini heading | ✅ 改为 `fmod(heading + M_PI, 2π)` 叠加 |
