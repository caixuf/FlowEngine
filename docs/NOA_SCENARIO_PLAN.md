...
> **状态：验证阶段（2026-07-18 更新）。** Phase 1-4 已完成，city_to_highway_full 场景
> 端到端可运行（25 entities / 7 roads / 2 junctions / 30s+ 无崩溃）。
> 剩余工作：场景格式统一迁移、perception↔sensor 数据流、planning 超车、3D 分叉渲染。
>
...
### Phase 6 — 质量收敛与能力补全 🟡 进行中

> **2026-07-18 更新**：基于 Phase 4 端到端验证 + 用户实际运行反馈，新增 pipeline 冻结、3D 车道渲染、topology 文件丢失等问题。优先级重排。
>
> **2026-07-17 进展**：merge 闭环、3D 分叉渲染、3D 轨迹渲染、超车感知补全、esmini 兜底、场景格式统一迁移、gltf 模型资产、demo.sh 旗舰场景默认化已完成。剩余 3D 车道宽度精确渲染（依赖 esmini per-road 查询）和逆向车 heading 叠加两项待后续。
