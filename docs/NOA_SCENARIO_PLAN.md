...
| 步骤 | 文件 | 产出 |
|------|------|------|
| 1.1 `json_to_xodr.py` 新增 junction 生成 | `tools/json_to_xodr.py` | 解析 `junctions` → 输出 `<junction>` + 连接道路 |
| 1.2 场景 JSON 定义 road_network + junctions | `scenarios/city_to_highway_full.json` | 7 段 road + 2 个 junction（fork + merge） |
| 1.3 验证 esmini 加载多段路网 | flowsim 启动日志 | `esmini road network loaded: N roads` 包含连接道路 |

### Phase 2 — 核心数据流修复（~2 天）

| 步骤 | 文件 | 产出 |
|------|------|------|
| 2.1 perception 支持 sensor/lidar 输入 | `modules/adas_nodes/perception_node.cpp` | 新增 `mode: sensor`，订阅 sensor/lidar |
| 2.2 monitor 透传完整 entities | `modules/adas_nodes/monitor_node.c` | `metrics.scene.entities` 包含全部 NPC |
| 2.3 3D 前端消费 entities | `tools/flowboard/js/scene3d.js` | 障碍物从 entities 渲染，池扩展到 24 |
| 2.4 验证回归 | `demo_evaluator.py` | 旧场景全部通过 |

### Phase 3 — planning 选路与汇入（~3 天）

| 步骤 | 文件 | 产出 |
|------|------|------|
| 3.1 scenario_loader 新增 branch_select / merge | `src/core/scenario_loader.c` | 解析 route type 字段 |
| 3.2 planning branch_select 处理 | `planning_node.cpp` | 分叉点自动切换 Frenet 参考线 |
| 3.3 planning merge 处理 | `planning_node.cpp` | 加速找间隙 → 并入主线 |
| 3.4 弯道控制增强 | `control_node.cpp` | R≤60m 时前馈权重加大 |

### Phase 4 — 场景编排与验证（~2 天）

| 步骤 | 产出 |
|------|------|
| 4.1 编写 `city_to_highway_full.json` | 完整场景（road_network + 24 NPC + route） |
| 4.2 预跑测试 | 逐个阶段手动验证（红灯/ETC/分叉/汇入/超车） |
| 4.3 回归验证 | `demo_evaluator.py` 全场景通过 |
| 4.4 调优参数 | PID、IDM 参数、摄像机角度 |

### Phase 5 — 可视化增强（~1 天）

| 步骤 | 文件 | 产出 |
|------|------|------|
| 5.1 分叉道路 3D 渲染 | `scene3d.js` | 两支道路都显示 |
| 5.2 NPC AI 状态标签 | `scene3d.js` | 障碍物上方显示 AI 状态（跟车/停车/巡航） |
| 5.3 规划轨迹渲染 | `scene3d.js` | 彩色虚线显示 planning 输出的路径 |

**总计：约 10 天（比原方案的 14 天省了 4 天，因为 flowsim v2 已覆盖大量基础工作）**
