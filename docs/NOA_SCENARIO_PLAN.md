...
> **状态：收尾阶段（2026-07-18 更新）。** Phase 1-6 主体已完成。city_to_highway_full 为默认旗舰场景，
> 24 NPC + 分叉/汇入 + 超车全链路可运行。剩余：3D 车道宽度精确渲染、逆向车 heading 叠加。
>
...
| 缺口 | 影响 | 优先级 | 状态 |
|------|------|--------|------|
| **3D 车道宽度精确渲染** | 道路渲染用全局 lane_width 单值，不同路段宽度不同 | 🟡 P1 | ⏳ 待 esmini per-road 查询 |
| **逆向车 heading 叠加** | `e.heading=M_PI` 无条件覆盖，弯道段可能朝向错误 | 🔵 P2 | ⏳ 低风险，交叉口直道不受影响 |
...
| 原始缺口 | 修复 | Phase |
|----------|------|-------|
| 分叉/汇入路网 | json_to_xodr.py junction 生成 | Phase 1 |
| planning branch_select | scenario_loader + planning 分叉选路 | Phase 3 |
| planning 汇入逻辑 | merge gap 检测 + 并入下发 | Phase 6 |
| perception 绕过 sensor_model | mode=sensor 订阅 sensor/lidar | Phase 6 |
| 3D 前端消费 entities | scene3d.js 池扩展到 24 | Phase 2 |
| 障碍物池上限 | MAX_OBS 4→8，entities 填充 | Phase 6 |
| monitor 截断数据 | entities/road_network 透传 | Phase 2 |
| pipeline 冻结 | control 感知补全 + 协程数据竞争修复 | Phase 6 |
| 场景格式不统一 | 12 场景迁移到 s/l/segment_id | Phase 6 |
| 无 glTF 模型 | sedan/truck/suv/pedestrian glTF 资产 | Phase 6 |
...
| 1.4 esmini 加载 | flowsim 启动日志 | ✅ 8 roads + 2 junctions，S-curve ramp 净方向变化=0 |
...
### Phase 4 — 场景编排与验证（~2 天）✅ 已完成 (2026-07-18)

| 步骤 | 状态 |
|------|------|
| 4.1 编写 `city_to_highway_full.json` | ✅ 7 段 road + 2 junction + 24 NPC + branch_select/merge route |
| 4.2 预跑测试 | ✅ 30s/45s/60s 无崩溃，esmini 加载成功 |
| 4.3 回归验证 | ✅ 19/25 NPC 移动，旧场景 highway_overtake 兼容 |
| 4.4 调优参数 | ✅ S-curve ramp 替代 hairpin；逆向车 heading=π；StopForTL→Cruise 恢复 |
| 4.5 缺陷修复 | ✅ 6 项修复 (commit b538bf1): merge junction xodr, NPC Frenet 定位, 编译错误 |
...
**进度：已完成 Phase 1-4，Phase 5 部分完成。下一阶段：Phase 6（场景统一 + planning + 3D 分叉）。**

### Phase 6 — 质量收敛与能力补全 ✅ 基本完成 (2026-07-18)

> **11/13 项已完成。** 剩余 2 项：3D 车道宽度精确渲染（依赖 esmini per-road 查询）、逆向车 heading 叠加。

| 优先级 | 任务 | 文件 | 现象 | 说明 |
|--------|------|------|------|------|
| 🔴 P0 | **pipeline 冻结排查** | `control_node.cpp` | demo 运行 ~28s 后 pub 计数不再增长，ego 速度=0，全链路静默 | ✅ 根因定位：vehicle/state 仅 4 槽位无法覆盖 24-NPC 场景前方慢车，control 被动超车逻辑因 best_gap=1e9 不触发。已修复：MAX_OBS 4→8，新增 on_scene_frame 订阅 scene/frame entities 填充槽位 4-7 |
| 🔴 P0 | **perception↔sensor 数据流对接** | `perception_node.cpp` | perception 读真值，sensor 噪声链路未验证 | ✅ 已实现 mode=sensor 订阅 sensor/lidar，用 _msg_cast_impl 解析 LidarFrame |
| 🔴 P0 | **场景格式统一迁移** | `scenarios/*.json`, `flowsim_node.cpp` | 旧格式 x/y 路径与 Frenet 路径共存，维护双份逻辑 | ✅ 12 个旧场景迁移到 segment_id/s/l 新格式；flowsim_node esmini 离线兜底用线性放置。旧 else 分支保留兼容 ego 全局坐标 |
| 🟡 P1 | **3D 车道渲染修正** | `scene3d.js`, `scene_pub.cpp` | 道路渲染与车辆位置视觉上不匹配，"车不跑在车道里" | ⏳ 待 esmini per-road 车道宽度查询接入（依赖 esmini 环境） |
| 🟡 P1 | **planning 超车能力** | `control_node.cpp` | ego 被同车道慢车阻挡时无法变道超车 | ✅ control 已有被动超车状态机（lc_state 0→1→2→3），Phase 6 补全感知后自动生效。fallback 模式（无 Frenet）下 route_lane 下发仍可触发变道 |
| 🟡 P1 | **分叉道路 3D 渲染** | `scene3d.js` | `road_network` 含 junctions 时，前端仅渲染顺序 edges，不显示分叉/汇入路网拓扑 | ✅ 前端端点聚类检测 junction（≥3 条 edge 共享端点），fork 标橙色环、merge 标绿色环 |
| 🟡 P1 | **规划轨迹 3D 渲染** | `scene3d.js`, `monitor_node.c` | 前端不显示 planning 下发的参考轨迹线 | ✅ monitor 透传 trajectory path 数组，scene3d 从 ego 位置+heading 做 Frenet→World 近似画蓝色轨迹线 |
| 🔵 P2 | **demo.sh topology 文件保留** | `scripts/demo.sh` | cleanup 中 `rm -f /tmp/flow_topology.json` 导致事后评估无数据 | ✅ 已在 commit 79b477c 修复，改为保留+时间戳副本 |
| 🔵 P2 | **esmini 加载失败兜底** | `flowsim_node.cpp` | `g.roads_loaded=false` 时 NPC 全部堆在原点 | ✅ segment_id≥0 的 actors 在 esmini 离线时线性放置（e.x=s, e.y=l+road_center_y），叠加弯道偏移 |
| 🔵 P2 | **逆向车 heading 冲突** | `flowsim_node.cpp` | `e.heading=M_PI` 无条件覆盖 esmini heading | ⏳ 当前仅交叉口直道使用，低风险；弯道段需改为叠加而非覆盖 |
| 🟢 新增 | **glTF 模型资产** | `tools/flowboard/models/*.gltf` | models/ 目录缺失，GLTFLoader 降级到 BoxGeometry | ✅ 生成 sedan/truck/suv/pedestrian 4 个占位 gltf（盒子网格+PBR 材质，1:1 米单位） |
| 🟢 新增 | **demo.sh 旗舰场景默认化** | `scripts/demo.sh`, `config/pipeline.json` | 默认场景为 congestion_follow（简单跟车），无法展示 NOA 全链路 | ✅ 默认改为 city_to_highway_full.json（24 NPC + 分叉 + 汇入 + 超车） |
| 🟢 新增 | **merge 完整闭环** | `planning_node.cpp` | merge 路线步骤仅强制速度+车道，无 gap 决策 | ✅ planning 订阅 scene/frame 拿主线来车，算前后 gap，gap 不足时跟车巡航不下发变道，gap 充足时下发并入 |
...
| 3D 前端是否需要 glTF 模型 | **已完成** | Phase 6 已生成 sedan/truck/suv/pedestrian 4 个 glTF 占位模型（PBR 材质） |
