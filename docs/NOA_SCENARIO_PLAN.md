...
### Phase 6 — 下一阶段：质量收敛与能力补全（~5 天）🔴 待开始

> **2026-07-18 更新**：基于 Phase 4 端到端验证 + 用户实际运行反馈，新增 pipeline 冻结、3D 车道渲染、topology 文件丢失等问题。优先级重排。

| 优先级 | 任务 | 文件 | 现象 | 说明 |
|--------|------|------|------|------|
| 🔴 P0 | **pipeline 冻结排查** | `planning_node.cpp`, `control_node.cpp` | demo 运行 ~28s 后 pub 计数不再增长，ego 速度=0，全链路静默 | ego 被慢车（vx=3.0）阻挡 → 减速至 0 → 无超车/恢复策略 → 管道卡死。需排查：是否死锁在某个节点的等待？是否 control_node 的 ROAD_GUARD 再次触发？ |
| 🔴 P0 | **perception↔sensor 数据流对接** | `perception_node.cpp`, `pipeline.json` | perception 读真值，sensor 噪声链路未验证 | 改为订阅 `sensor/lidar`，打通传感器噪声/FOV/遮挡链路 |
| 🔴 P0 | **场景格式统一迁移** | `scenarios/*.json`, `flowsim_node.cpp` | 旧格式 x/y 路径与 Frenet 路径共存，维护双份逻辑 | 14 个旧场景迁移到 `s/l/segment_id`；删除 `else` 分支，统一为 Frenet→World 单一路径 |
| 🟡 P1 | **3D 车道渲染修正** | `scene3d.js`, `scene_pub.cpp` | 道路渲染与车辆位置视觉上不匹配，"车不跑在车道里" | `cfg.lane_width` 用全局单值；不同道路车道数/宽度不同。需从 esmini per-road 查询实际车道宽度传入 topology |
| 🟡 P1 | **planning 超车能力** | `planning_node.cpp` | ego 被同车道慢车阻挡时无法变道超车 | 需在 `lane_change` route step 基础上支持基于障碍物的主动变道决策 |
| 🟡 P1 | **分叉道路 3D 渲染** | `scene3d.js` | `road_network` 含 junctions 时，前端仅渲染顺序 edges，不显示分叉/汇入路网拓扑 | 需读取 junction 连接关系，在 fork/merge 点渲染分支道路 |
| 🟡 P1 | **规划轨迹 3D 渲染** | `scene3d.js` | 前端不显示 planning 下发的参考轨迹线 | — |
| 🔵 P2 | **demo.sh topology 文件保留** | `scripts/demo.sh` | cleanup 中 `rm -f /tmp/flow_topology.json` 导致事后评估无数据 | ✅ 已在 commit 79b477c 修复，改为保留+时间戳副本 |
| 🔵 P2 | **esmini 加载失败兜底** | `flowsim_node.cpp` | `g.roads_loaded=false` 时 NPC 全部堆在原点 | 应至少用线性累积 x=s 放置 |
