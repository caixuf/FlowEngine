...
当前 `ctest` 全量运行存在一个问题：`flow_task_test` 会卡住，因为 `flow_task` 是交互式 demo，不适合作为自动化测试。

排除 `flow_task_test` 后，其余测试全部通过：
...
`flow_task_test` 会卡住，因为它对应的是交互式程序。

建议：

- 保留 `flow_task` 作为交互 demo。
- 新增非交互 `flow_task_smoke_test`。
- 或给 `flow_task` 增加 `--test --duration 3` 模式。
- 给每个 CTest 设置合理 `TIMEOUT`。
...
- 移除或改造 `flow_task_test`。
- 给 demo 类测试设置 `TIMEOUT`。
- 增加 smoke test 模式。
