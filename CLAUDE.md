...
| 变道不丝滑（先减速后慢慢变道） | 变道目标速度锁死在入口值 `fmax(adj_speed, ego_v)`，目标车道前车慢则整个变道被压速（实测 tgt=11.89 全程不加速）；变道评估只看目标车道 gap 够不够、不看是否更快 → 3→2→3 变进同样慢车道后立刻 FOLLOW 减速。修复：目标车道前车 > `lc_merge_clear_m`(80m) 时变道目标用巡航速度（加速/平速变道，实测 tgt=20.00、v 12.7→15.9 加速通过）；变道加"有用"门 `target_lane_useful`（目标车道前车 ≥ 本车道前车+2m/s 才变，防变进慢车道循环） | `behavior_planner_node.cpp` adj_speed / target_lane_useful |
