...
| 变道不丝滑（先减速后慢慢变道） | 变道评估只看目标车道 gap 够不够、不看是否更快 → 3→2→3 变进同样慢车道后立刻 FOLLOW 减速（实测变道后跟车压速 10 m/s）。修复：变道加"有用"门 `target_lane_useful`（目标车道前车 ≥ 本车道前车+2m/s 才变，防变进慢车道循环）；变道目标速度保持 `max(adj_speed, ego_v)`（平速/按目标车道前车限速，不减速变道）。曾尝试"目标车道空旷即加速到巡航"——实测加速到 20 逼近远处前车（min_forward_gap 6.9m 接近碰撞）+ 高速变道横向过冲，回退 | `behavior_planner_node.cpp` target_lane_useful |
| 变道时车一顿一顿（横向振荡） | 变道轨迹首点 heading 设为 ego_heading（修 ribbon"脱离车头"）后，control `query_ref_at` 取最近点=point[0]（车自身）→ ref_road_heading=车头朝向 → 横向 heading 阻尼项清零 → 纯 P 控制限环振荡（车横向一顿一顿）。修复：`query_ref_at` 从 index 1 取参考点（轨迹切线，S 曲线），保留 heading 阻尼；ribbon 的"脱离车头"修复仍在（首点 heading 不变，仅控制参考跳过首点） | `control_node.cpp` query_ref_at |
| 掉头时倒车腾挪（"掉头直接倒车"不真实） | 掉头触发时前向空间 < `min_uturn_space`(12m) → Phase 0 直线倒车腾挪。实测 fwd=11.5m 只差 1.5m 就触发倒车（观感太骚）。修复：`min_uturn_space` 12→10（掉头首弧 2×R≈7.9m，10m 足够开始多把方向）；另 FOLLOW 状态掉头 approach 区用 `uturn_max_trigger_speed` 覆盖跟车目标（否则被慢车拖到路端、触发时前向空间不足） | `planning_node.cpp` min_uturn_space / `behavior_planner_node.cpp` FOLLOW 掉头 approach 覆盖 |
| 变道中轨迹脱离车头（还没变过去轨迹就变直） | 变道 S 曲线五次多项式 a1=a2=0（起点横向速度 0）→ 轨迹永远从平行路面的 0 横向速度出发，而车在变道中已偏转 → 轨迹平直脱离车头；且按点索引参数化 + 固定 L=50m，低速下在 5s 视界内走不完。修复（A/B 二分定位）：**h0 切线连续经实测为横向过冲来源**（评估器 y 范围 FAIL，车冲过目标车道 0.87m）→ 回退 h0=0 原解，车头朝向改由**轨迹首点 heading=ego_heading** 单独贴合（ribbon 从车头出发，位置仍沿平缓 S 曲线 → 无过冲）；L = `clamp(ego_v×5, 50, 70)` = 5s 视界长度（匹配旧点索引实际平缓度，S 曲线在轨迹内完成无截断）；`k_vy_damp` 0.4→0.6（更强横向速度阻尼，压过冲峰值，评估器 PASS） | `planning_node.cpp` 变道 d_out 五次多项式 + heading 贴合 / `config/pipeline.json` k_vy_damp |
| 掉头先停再慢转 + 返程掉头卡死 | 掉头 Phase 1 只刹到 uturn_speed 就进弯（进弯带初速撞护栏）；刹车到 0 后 PID **负积分残留**把车钉在原地（返程掉头实测 spd=0 target=0.5 brk=0.13 卡死 30s→timeout）——error 翻负积累的负积分在目标翻正后不被 anti-windup 清除。修复：Phase 1 刹到 0 + 驻停 0.5s；uturn_speed 3.5→2.5、reverse -3.0→-2.0（Python 仿真 corner_max 6.00 距路沿 1.0m）；maneuver_tracker speed_floor 1.5→0.5；control 机动模式车近停且 target 为正时清负积分（负积分对称 anti-windup） | `planning_node.cpp` generate_uturn_trajectory / `control_node.cpp` anti-windup |
| 掉头后幽灵刹车（返程遇对向车硬刹） | safety `min_oncoming_ttc` 横向窗 `(2.0, 6.0]m` 把**相邻对向车道**（间隔 3.5m 的分隔车道）当迎头威胁 → 返程（heading≈π 朝西）每次接近东行对向车 head-on TTC 硬刹；planning 会车让行用世界 dx 窗口 `(-10,80]` 返程方向盲。修复（用户选定"同车道头对头才刹"）：`min_oncoming_ttc` 横向窗 → `[0, 2.3]`（只同车道真头对头，60m 预警保留；分隔车道不再误伤）；planning 会车让行世界 dx → 沿车头投影 + 同车道对齐（方向通用）；同车道头对头另有 min_vehicle_ttc(35m) 兜底 | `safety_control_node.cpp` min_oncoming_ttc / `planning_node.cpp` 会车让行 |
| 写死固定朝向车道（方向不通用） | 多处硬编码 ±1.75 / 世界 +x 坐标：`generate_uturn_trajectory` 目标车道 `±lane_width*0.5`（只对 4 车道对称路成立）；`control_node` 出路沿恢复 `-1.75`（返程被拽向对向侧）；safety 两个行人函数用世界 dx 判前/后（掉头后方向反）。修复：uturn 目标车道用 `lane_center_y(lc/2-1 或 lc/2)` 推导（任意车道数自洽）；control 恢复 → ego 所在侧第一条车道 `road_center ± 半车道`；行人函数 → 沿车头投影 | `planning_node.cpp` target_lane_center_y / `control_node.cpp` 出路沿恢复 / `safety_control_node.cpp` 行人函数 |
