...
function _createGrassColorTexture() {
  ...
  for (let y = 0; y < TEXTURE_SIZE; y++) {
    for (let x = 0; x < TEXTURE_SIZE; x++) {
      ...
      // 基础草绿 + 随机深浅变化
      const variation = (Math.random() - 0.5) * 30;  // -15 ~ +15
      const r = Math.max(0, Math.min(255, 100 + variation));
      const g = Math.max(0, Math.min(255, 180 + variation));
      const b = Math.max(0, Math.min(255, 60 + variation * 0.5));
      data[i] = r;
      data[i + 1] = g;
      data[i + 2] = b;
      data[i + 3] = 255;
  ...
  // 可选：叠加草叶纹理（细短线）
  ctx.strokeStyle = 'rgba(50, 120, 30, 0.15)';
  ctx.lineWidth = 1;
